...
class Node(object): #Esta classe descreve apenas um nódo!
    def __init__(self, x, y):
        self.position = Vector2(x, y) #Nódos atuam com vetores
        self.neighbors = {UP:None, DOWN:None, LEFT:None, RIGHT:None, PORTAL:None} #Ligações (vetores) com outros nódos (vizinhos)
        self.access = {UP:[PACMAN, BLINKY, PINKY, INKY, CLYDE, FRUIT], #Quem pode acessar tal nódo
                       DOWN:[PACMAN, BLINKY, PINKY, INKY, CLYDE, FRUIT],
                       LEFT:[PACMAN, BLINKY, PINKY, INKY, CLYDE, FRUIT],
                       RIGHT:[PACMAN, BLINKY, PINKY, INKY, CLYDE, FRUIT]}

    def denyAccess(self, direction, entity): #Negar acesso ao nódo (remover do dicionário)
        if entity.name in self.access[direction]:
            self.access[direction].remove(entity.name)

    def allowAccess(self, direction, entity): #Permitir acesso ao nódo (adicionar no dicionário)
        if entity.name not in self.access[direction]:
            self.access[direction].append(entity.name)

    def render(self, screen): #Desenhar o nodo e as ligações na tela
        for n in self.neighbors.keys():
            if self.neighbors[n] is not None:
                line_start = self.position.asTuple() #Posição do nodo
                line_end = self.neighbors[n].position.asTuple() #Posição do nodo vizinho
